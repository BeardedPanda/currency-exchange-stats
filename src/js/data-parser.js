...
const parseRulya = () => fetch(bankUrls.rulya).then(res => res.text());
export const updateRulya = (skipNotifications = false) =>
	parseRulya()
		.then(response => {
			const doc = new DOMParser().parseFromString(response, "text/html");

			return [...doc.querySelectorAll("#ltblBAK tr:not(:first-child)")].map(tr => ({
...
const parsePiramida = () => fetch(bankUrls.piramida).then(res => res.text());

export const updatePiramida = (skipNotifications = false) =>
	parsePiramida()
		.then(response => {
			const doc = new DOMParser().parseFromString(response, "text/html");
...
const parseLion = () => fetch(bankUrls.lion).then(res => res.text());

export const updateLion = (skipNotifications = false) =>
	parseLion()
		.then(response => {
			const doc = new DOMParser().parseFromString(response, "text/html");
...
const parseGoverla = () => fetch(bankUrls.goverla).then(res => res.text());

export const updateGoverla = (skipNotifications = false) =>
	parseGoverla()
		.then(response => {
			const doc = new DOMParser().parseFromString(response, "text/html");
			return [...doc.querySelectorAll("#rates .gvrl-table .gvrl-table-body .gvrl-table-row")].map(tr => ({
